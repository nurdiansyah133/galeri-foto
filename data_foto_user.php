...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['editfoto'])) {
    ...
    if ($update) {
        ...
        echo '<script>window.location="data_foto_user.php"</script>';
    } else {
    ...
    <section id="hero" class="hero d-flex flex-column justify-content-center align-items-center" data-aos="fade"
        ...
        <div class="col-md-9">
            ...
            <div class="container">
                <div class="row">
                    <div class="col">
                        <form action="data_foto_user.php" method="GET" class="d-flex justify-content-start">
                            <div class="input-box">
...
$total_records_query = mysqli_query($conn, "SELECT COUNT(*) FROM tb_galeri WHERE user_id = '$iduser'");
...
// Query utama untuk mengambil data sesuai dengan batasan limit dan offset serta id user
$query = "SELECT * FROM tb_galeri WHERE user_id = '$iduser'";

// Tambahkan kondisi WHERE jika ada keyword pencarian
if (!empty($keyword)) {
    $query .= " AND (galeri_name LIKE '%$keyword%' OR admin_name LIKE '%$keyword%')";
}
                    ...
                    if (mysqli_num_rows($cek) > 0) {
                        while ($d = mysqli_fetch_object($cek)) {
                            ...
                            >Edit</button>
        <a href='data_foto_user.php?id=" . $d->galeri_id . "' class='btn btn-danger' onclick='return confirm(\"Apakah Anda yakin ingin menghapus pengguna ini?\")'>Hapus</a> </td>";
